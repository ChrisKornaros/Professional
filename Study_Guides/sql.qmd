...
- For the first part:
  ...
  - GROUP BY user_id and name to count ratings per user
...
- For the second part:
  ...
  - GROUP BY movie_id and title to calculate average rating per movie
...
```sql
-- Write your PostgreSQL query statement below
SELECT visited_on, SUM(cumount) OVER (ROWS BETWEEN 6 PRECEDING AND CURRENT ROW) AS amount, ROUND(SUM(cumount) OVER (ROWS BETWEEN 6 PRECEDING AND CURRENT ROW)::numeric/7, 2) AS average_amount
FROM (
...
- The inner subquery first calculates the total daily revenue by grouping the Customer table by visited_on.
...
- This approach counts each person once for each friendship they're involved in, whether as a requester or accepter.
...
- For large datasets, indexes on requester_id and accepter_id would improve the performance of the UNION ALL operation.
...
- Time complexity: O(n²) due to the correlated subqueries, where n is the number of insurance policies.
...
- The solution correctly handles the combination of conditions where we need both a match (same tiv_2015) and non-match (unique location).
- For large datasets, indexes on tiv_2015, lat, and lon would significantly improve the performance of the subqueries.
...
- Time complexity: O(n²) in the worst case due to the self-join, where n is the number of person records.
